...
renderer.outputColorSpace = THREE.SRGBColorSpace; // r180
...
function createSignBoardPlane({
  ...
  ctx2d.fillStyle = bg;
  const r = corner; const w = cw; const h = ch;
  ctx2d.beginPath();
  ctx2d.moveTo(r, 0);
...
function attachSignText(signMesh, text, size, material = textMatWhite, zOffset = 2) {
  if (!uiFont) return null;
  const g = new TextGeometry(text, { font: uiFont, size, depth: 0, bevelEnabled: false });
  g.center();
...
const audio1URL = '/3d/audio/roku1.mp3';
const audio2URL = '/3d/audio/roku2.mp3';
const bgmURL    = '/3d/audio/psy.mp3';
...
// MediaElementSource は各HTMLMediaElementにつき一度だけ！
...
gainReverb1.gain.value = 0.0; // 初期ドライ
...
// メインアウト
const gainBus1  = actx.createGain();  gainBus1.gain.value = 1.0;
...
// 任意のローパス（将来用。今は未配線）
const lowpassFilter1 = actx.createBiquadFilter(); lowpassFilter1.type = 'lowpass'; lowpassFilter1.frequency.value = 22050;
const lowpassFilter2 = actx.createBiquadFilter(); lowpassFilter2.type = 'lowpass'; lowpassFilter2.frequency.value = 0;
...
function setEchoSend(v, durSec=0.8){ rampParam(echoSendGain.gain, v, durSec); }

// 視覚サイケON/OFFに合わせて音も深リバーブ+エコー
...
function animateCameraToPositionAndTarget(cam, targetPos, targetLookAt, durationMs, onDone) {
  const startPos = _v3a.copy(cam.position);
  const startLookAt = cam.getWorldDirection(_v3b).add(cam.position);
  let startTime = null;

  function update(t) {
    if (!startTime) startTime = t;
    const p = Math.min((t - startTime) / durationMs, 1);
    _v3c.copy(startPos).lerp(targetPos, p);
    const look = _v3b.copy(startLookAt).lerp(targetLookAt, p);
    cam.position.copy(_v3c);
    cam.lookAt(look);
...
function handleClick(event) {
  ...
  const intersects1 = _raycaster.intersectObjects(clickableA.concat([bubble1, mBubble2, bubble3, bubble4, bubble5]), true);
  if (intersects1.length) {
    const obj = intersects1[0].object;
    ...
    if (obj === menuPlane) {
     ...
     animateCameraToPositionAndTarget(camera, new THREE.Vector3(0, 20, 20), new THREE.Vector3(0, 0, -1000), 3000);
      ...
      animateCameraToPositionAndTarget(camera, toPos, toLookAt, 3000, () => {
        ...
        setTimeout(() => {
          window.location.assign('https://anjiteraoka.com/3d/rb.html');
        }, 2200);
      });
      handled = true;
    ...
    // ANJI TERAOKAをクリック
      ...
      animateCameraToPositionAndTarget(camera, toPos, toLookAt, 3000, () => {
        ...
        rainbowStartTime = performance.now();
        setTimeout(() => {
          window.location.assign('https://anjiteraoka.com/3d/at.html');
        }, 2200);
      });
      handled = true;
    }
