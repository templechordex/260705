...
import { addObjeToScene } from './rbobje.js';
...
// --------------------------------------
const { scene, camera, renderer } = createSceneCameraRenderer(THREE, { exposure: 0.1 });
...
gltfLoader.load('model/ele.glb', (gltf) => {
  ele = gltf.scene;
  ele.scale.set(2600, 2600, 2600);
  ele.position.set(0, 100, 0);
...
const flagGeometry = new THREE.PlaneGeometry(760, 1260, 12, 6);
const flagMesh = new THREE.Mesh(flagGeometry, flagMaterial);
flagMesh.position.set(0, 100, -1000);
flagMesh.rotateX(Math.PI / 1);
flagMesh.rotateY(Math.PI / 1);
scene.add(flagMesh);
...
// --------------------------------------
const dirLight = new THREE.DirectionalLight(0xb0d8ff, 10);
dirLight.position.set(0,200, 30);
scene.add(dirLight)
...
// 暖色ライト群
const warmKey = new THREE.DirectionalLight(0xffcc66, 10);
warmKey.position.set(100, 0, -50);
scene.add(warmKey);
...
const signB = createSignBoardPlane({
  width: 10, height: 3, bg: 'rgba(10,10,30,0.25)', glow: '#bb66ff'
});
signB.position.set(-102, -24, 0);
signB.rotation.y = THREE.MathUtils.degToRad(20);
scene.add(signB);

// ネオン無しの「戻る」看板（クリックで y=-1000 → y=0）
const signReturn = createSignBoardPlane({
  width: 260, height: 90, bg: 'rgba(5,5,10,0.35)', glow: null
});
signReturn.position.set(0, -800, -1100);
...
function buildSignTexts() {
  ...
  if (!signReturnText)signReturnText= attachSignText(signReturn, 'BACK TO TOP', 18, textMatWhite, 2);
}
...
function handleClick(event) {
  ...
  if (intersects1.length) {
    ...
    if (obj === menuPlane) {
      ...
      const toPos    = new THREE.Vector3(0, 30, 1600);
      const toLookAt = new THREE.Vector3(0, 0, -100);
